...
# qss, Start training
# critic_num = 5 if config['dis']['gan_type'] == 'wgan-gp' else 1
...
while True:
    for it, (images, h0_labels, h1_labels, h2_labels, h0_local_labels, h1_local_labels) in enumerate(train_loader):
        ...
        with Timer("Elapsed time in update: %f"):
            trainer.dis_update(images, config, h0_local_labels, h1_labels, h1_local_labels, h2_labels, h2_labels)
            trainer.gen_update(images, config, h0_local_labels, h1_labels, h1_local_labels, h2_labels, h2_labels)
        ...
        if (iterations + 1) % config['image_save_iter'] == 0:
            with torch.no_grad():  # close the grad compute when test
                te_output = trainer.rand_trans(test_display_data, test_display_h0_local_labels, test_display_h1_labels,
                                       test_display_h1_local_labels, test_display_h2_labels, test_display_h2_labels)
                tr_output = trainer.rand_trans(images, h0_local_labels, h1_labels,
                                       h1_local_labels, h2_labels, h2_labels)

                write_2images(test_display_data, test_display_data[te_output[0]], te_output[1], te_output[2],
                              [test_display_h0_local_labels, test_display_h1_labels, test_display_h1_local_labels,
                               test_display_h2_labels], te_output[3:], image_directory, 'test_%08d' % (iterations + 1),
                              config['w_scale'], config['h_scale'])
                write_2images(images, images[tr_output[0]], tr_output[1], tr_output[2], [h0_local_labels,
                              h1_labels, h1_local_labels, h2_labels], tr_output[3:],
                              image_directory, 'train_%08d' % (iterations + 1), config['w_scale'], config['h_scale'])
